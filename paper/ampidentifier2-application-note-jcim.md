...
Tool availability is a persistent concern in this field. AMP predictors distributed exclusively as web applications are vulnerable to server discontinuation, complicating long-term reproducibility and independent benchmarking. Tools distributed through locally executable packages and open-source repositories with versioned model artifacts are less susceptible to this failure mode and can be integrated into automated pipelines without dependency on external server infrastructure.
...
The AMPidentifier web application at https://www.ampidentifier.com provides a browser-based interface that does not require local software installation. Users paste or upload sequences in FASTA format, select a model, and download the two output CSV files. The web server runs the same pre-trained models as the CLI and PyPI package, producing identical predictions for any given input. The interface is accessible from any device with a browser and an internet connection, including mobile devices, making it suitable for exploratory use and for users without programming experience (Figure 3).
...
**Figure X.** Grouped bar chart comparing the six AMPidentifier configurations on the internal held-out test set ($n$ = 2,650; 1,325 AMP and 1,325 non-AMP sequences; 20% stratified partition withheld from all training and hyperparameter optimization steps). Seven metrics are shown: Accuracy, Precision, Sensitivity (Sn), Specificity (Sp), F1, MCC, and AUC-ROC. All configurations use teal shades from the same color family, with the Voting ensemble in the darkest tone and SVM in the lightest. Decision thresholds are the MCC-optimized values determined on this partition after training (Voting: 0.56; LGBM: 0.71; RF: 0.56; GB: 0.55; XGB: 0.48; SVM: 0.47). The Voting ensemble achieves the highest MCC (0.859) and AUC-ROC (0.977); LGBM is the strongest individual classifier (MCC 0.855, AUC-ROC 0.975). Dashed reference lines mark 70%, 80%, and 90% score thresholds. Acc: accuracy; Sn: sensitivity; Sp: specificity.

![Figure X: Internal held-out test set metrics](../benchmarking/fig_internal_metrics.png)
...
**Figure Y.** Grouped bar chart comparing all 15 classifier configurations evaluated on the independent benchmark ($n$ = 4,736; 2,368 AMP and 2,368 non-AMP sequences; Bhangu et al., 2025[22]). Seven metrics are shown: Accuracy, Precision, Sensitivity (Sn), Specificity (Sp), F1, MCC, and AUC-ROC. Bars are color-coded by tool family: AMPidentifier configurations in teal shades (darkest = Voting ensemble, progressively lighter through LGBM, RF, GB, XGB, to SVM), CAMPR3 configurations in violet shades (darkest = RF, lightest = ANN), and remaining tools (AMPScanner v2, AMPlify, ampir, DBAASP, amPEPpy) each in a distinct color. Hatched bars (N/A) indicate tools with binary-only output that do not produce probability scores and therefore have no AUC-ROC. Dashed reference lines mark 70%, 80%, and 90% score thresholds. AMPidentifier LGBM achieves the highest individual MCC (0.749) and the Voting ensemble the highest AUC-ROC (0.950) among all evaluated configurations. Acc: accuracy; Sn: sensitivity; Sp: specificity.

![Figure Y: Independent benchmark metrics comparison](../benchmarking/fig_benchmark_comparison.png)
...
AMPidentifier shares a structural limitation with all sequence-based AMP classifiers trained on curated databases: the training data over-represents well-characterized AMP families from a limited set of organisms and activity classes. The 22-descriptor feature vector captures physicochemical and compositional properties of canonical AMPs but does not encode structural context, post-translational modifications, or activity-determining tertiary interactions. Consequently, performance on AMPs with non-canonical scaffolds, atypical amino acid compositions, or biological activities outside the training distribution may deviate from the benchmark estimates reported here. AMP probability values from AMPidentifier should therefore be interpreted as a rank-order screening score rather than a quantitative activity measure; experimental validation of predicted candidates remains the required confirmation step before functional annotation. Future versions may address these limitations through integration of structure-aware representations, larger and more taxonomically diverse training sets, or sequence embedding models pre-trained on broad protein sequence corpora, which have shown improved generalization on underrepresented AMP families in recent studies.

## Summary and conclusions

AMPidentifier provides sequence-based AMP prediction through five independently accessible machine learning classifiers and a soft-voting ensemble. The training dataset contains 13,246 balanced sequences; features are a 22-descriptor set covering global peptide properties, functional group fractions, and positional FET and solvent accessibility distributions; LightGBM is included as a fifth classifier. On an internal test set of 2,650 sequences, the voting ensemble achieves accuracy 92.9%, MCC 0.859, and AUC-ROC 0.977. On an independent benchmark of 4,736 sequences, the ensemble achieves MCC 0.742 and AUC-ROC 0.950, exceeding AMPScanner v2 (MCC 0.718) and all other accessible comparators. Nine of nine web-only tools surveyed were inaccessible at evaluation time; AMPidentifier is distributed through three deployment modes operating on identical model artifacts to ensure long-term availability. Per-sequence physicochemical descriptor tables are produced alongside binary predictions in all deployment modes. AMPidentifier is available as a command-line tool (`ampidentifier`), a PyPI package (`pip install ampidentifier`, Python >= 3.10), and a web application at https://www.ampidentifier.com.
