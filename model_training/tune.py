...
# Search spaces
# ---------------------------------------------------------------------------
def get_search_spaces(X_train_robust, X_train_qt):
    """
...
def main():
    ...
    X_test = {"robust": X_test_robust, "qt": X_test_qt}

    cv     = StratifiedKFold(n_splits=CV_FOLDS, shuffle=True, random_state=RANDOM_STATE)
    spaces = get_search_spaces(X_train_robust, X_train_qt)
    ...
    report_lines = [
        ...
        f"Features     : 159 selected (selected_features.txt)",
        "",
    ]
    ...
    for name, cfg in spaces.items():
        ...
        search = RandomizedSearchCV(
            ...
            scoring=SCORING,
            cv=cv,
            n_jobs=N_JOBS if name != "stack" else 1,
            random_state=RANDOM_STATE,
            verbose=2,
            refit=True,
        )
        ...
        print(f"  Threshold (MCC)  : {threshold:.2f}")
    ...
    print("\n--- Tuning complete ---")
    print(pd.DataFrame(summary_rows)[["model","auc_roc","mcc","f1","threshold"]].to_string(index=False))
